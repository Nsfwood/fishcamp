...
//        let entry = SimpleEntry(date: Date(), configuration: configuration)
    ...
    func getTimeline(for configuration: SelectParkIntent, in context: Context, completion: @escaping (Timeline<Entry>) -> ()) {
        ...
        fetchAlertsForWidget(park: configuration.parkCode ?? "YOSE", completion: { (result) in
            ...
            VStack(alignment: .leading) {
                ...
                HStack {
                    ...
                    Text("+\(entry.numberOfAlerts - 1)").foregroundColor(.white).opacity(0.5)
                        .widgetURL(URL(string: "https://www.nps.gov/\(entry.configuration.parkCode ?? "yose")/index.htm")!)
                }
            }
            Spacer()
        }
